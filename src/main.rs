...
fn main() {
    ...
    app.insert_resource(sim::GenState { births_blocked: Default::default(),
        generation: 0,
    ...
    if let Some(path) = scenario_path {
        ...
        if has_creatures {
            app.add_systems(Update, (sim::live_step, sim::predation_step, sim::plant_step, sim::apply_growth_grants, sim::rot_step, scenario::scenario_step).chain());
        } else {
            app.add_systems(Update, (sim::plant_step, sim::apply_growth_grants, sim::rot_step, scenario::scenario_step).chain());
        }
        app.run();
        return;
    }
